...
def build_graphs_for_subject(
    ...
    Prints a connectivity summary so you can spot a threshold that is
    too aggressive (mean_edges ≈ 0 means the GNN has nothing to aggregate).
    Rule of thumb for PLV: threshold ≤ 0.3. For Pearson: threshold ≤ 0.5.
    """
    adj = build_adjacency(windows, fs, method=adj_method, plv_bands=plv_bands)

    if verbose:
        # Report pre-threshold connectivity so threshold choice is visible
        C = adj.shape[1]
        max_edges = C * (C - 1) / 2
        mean_plv = adj[adj > 0].mean() if (adj > 0).any() else 0.0
        print(f"  [{adj_method.upper()}] pre-threshold: "
              f"mean_weight={mean_plv:.3f} | "
              f"max_weight={adj.max():.3f} | "
              f"pct_above_thresh={100*(adj > (threshold or 0)).mean():.1f}%")

    adj = threshold_adjacency(adj, threshold=threshold,
                              keep_top_k_percent=keep_top_k_percent)

    if verbose:
        # Report post-threshold edge density — if near 0, threshold is too high
        n_edges_per_graph = (adj > 0).sum(axis=(1, 2)) / 2  # upper triangle
        print(f"  Post-threshold: mean_edges={n_edges_per_graph.mean():.1f} | "
              f"min_edges={n_edges_per_graph.min():.0f} | "
              f"isolated_graphs={(n_edges_per_graph == 0).sum()}/{len(adj)}")
        if n_edges_per_graph.mean() < 2:
            print(f"  ⚠️  WARNING: mean_edges < 2 — threshold={threshold} is too "
                  f"aggressive for {adj_method}. Lower it to 0.1–0.2 for PLV, "
                  f"0.2–0.4 for Pearson.")
