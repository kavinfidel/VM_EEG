...
class ExperimentConfig:
    ...
    l_freq: float = 10.0            # high-pass cutoff for analysis band
    h_freq: float = 80.0            # low-pass cutoff
...
def evaluate_all(train_dict, config: ExperimentConfig):
    ...
    for clf_type in config.classifiers:
        ...
        for test_subject in all_subjects:
            ...
            ax.set_title(f"{test_subject} | {clf_type} | acc={acc:.3f}")
            plt.tight_layout()

            if config.save_confusion_matrices:
                os.makedirs(config.output_dir, exist_ok=True)
                path = os.path.join(config.output_dir, f"cm_{clf_type}_LOSO_{test_subject}.png")
